...
// local
#include "mocked-sound-handler.hpp"
...
public:
    ...
    void SetUp() override {
        ...
        EXPECT_CALL(*soundHandler, acquireWriteHandle(testing::_, testing::_))
            .WillRepeatedly([&](auto config, auto listener){ 
                return std::make_shared<laar::MockedSoundHandler::MockWriteHandle>(config, listener);
            });
        EXPECT_CALL(*soundHandler, acquireReadHandle(testing::_, testing::_))
            .WillRepeatedly([&](auto config, auto listener){ 
                return std::make_shared<laar::MockedSoundHandler::MockReadHandle>(config, listener);
            });
        ...
        server = srv::Server::configure(SharedCallbackQueue, threadPool, configHandler, soundHandler);
...
public:
    std::shared_ptr<laar::MockedSoundHandler> soundHandler 
        = std::make_shared<laar::MockedSoundHandler>(nullptr);
